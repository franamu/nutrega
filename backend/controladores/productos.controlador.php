	...
	static public function ctrSubirMultimedia($datos, $ruta){
		...
		if(isset($datos["tmp_name"]) && !empty($datos["tmp_name"])){
			...
			if($datos["type"] == "image/jpeg"){
				...
				$maxCiclo = 50;
				$contador = 0;
				while(($estado) || ($contador <= $maxCiclo) ){

			       if (file_exists($rutaMultimedia)){
                                            
                       $nuevoNombre = rand(1,50);

                       $rutaMultimedia = $directorio."/".$nuevoNombre.".jpg";
			...
			if($datos["type"] == "image/png"){
				...
				while(($estado) || ($contador <= $maxCiclo) ){
			       ...
			       if (file_exists($rutaMultimedia)){
                       ...
                       $rutaMultimedia = $directorio."/".$nuevoNombre.".png";
